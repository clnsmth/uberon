# Setup and Verification Guide for `@clnsmth-ontology-agent`

This guide outlines the manual configuration steps and tests required to transition the Uberon fork's agentic workflows to Google Gemini and run under a secure, dedicated **GitHub App** named `@clnsmth-ontology-agent`.
...
2. **App Details:**
   * **GitHub App name:** Enter `clnsmth-ontology-agent`.
...
5. **Expected Outcome:**
   ...
   * The agent posts a comment on the specified issue with a status update, answers the question, and appends the standard `@clnsmth-ontology-agent` signature block.

---

### Test Case 2: Triggering via Issue Comment Mention
This verifies that the GitHub comment listener parses the custom `@clnsmth-ontology-agent please` trigger correctly. Since it's a GitHub App, it also matches if you type `@clnsmth-ontology-agent[bot] please`.

1. Open or create a test issue on your repository.
2. Post a comment on the issue with the trigger phrase:
   ```markdown
   @clnsmth-ontology-agent please quick question: what is the active git branch name?
   ```
...
1. Open an issue on your repository to edit/create a term. For example:
   ...
   > Body: @clnsmth-ontology-agent please update the definition of epiglottic cartilage to match the text in GEMINI.md.
2. **Expected Outcome:**
   * The agent reacts with 👀 and posts a `Working on it...` message.
   * The workflow runs in the ODK container (`obolibrary/odkfull`).
   * The agent:
     1. Creates a new branch named `clnsmth-ontology-agent-issue-<number>-run1`.
     ...
     6. Commits and signs off with `@clnsmth-ontology-agent`.
...
| :--- | :--- | :--- |
| **Workflow doesn't trigger on comments** | Trigger format error, unauthorized user, or workflow not on default branch. | Verify you are posting as `clnsmth` (or a username in `ai-controllers.json`) and using `@clnsmth-ontology-agent please` or `@clnsmth-ontology-agent[bot] please`. Ensure `ai-agent.yml` is merged into the default branch. |
